# =============================================================================
# TWSA Result Tab Sub-Module (one instance per analysis type)
# =============================================================================

#' TWSA Result Tab UI
#' @param id Module namespace ID.
#' @keywords internal
twsaResultTabSidebarUI <- function(id) {
  ns <- shiny::NS(id)
  shiny::tagList(
...
#' TWSA Result Tab Server
#' @param id Module namespace ID.
#' @param analysis_type Fixed string: "outcomes", "costs", "nmb", "ce", or "vbp".
#' @param twsa_results Reactive containing TWSA results.
#' @param metadata Reactive containing model metadata.
#' @keywords internal
twsaResultTabServer <- function(id, analysis_type, twsa_results, metadata) {
  shiny::moduleServer(id, function(input, output, session) {
    ...
    output$controls <- shiny::renderUI({
      ...
      # TWSA name selector when multiple analyses exist
      twsa_names <- unique(
        res$twsa_metadata$twsa_name[!is.na(res$twsa_metadata$twsa_name)]
      )
      ...
      if (length(twsa_names) > 1) {
        inputs <- c(inputs, list(
          shiny::selectInput(ns("twsa_name"), "TWSA Analysis",
            choices = twsa_names,
            selected = twsa_names[1]
          )
        ))
      }
    ...
    selected_twsa_name <- shiny::reactive({
      res <- twsa_results()
    ...
    output$result_plot <- shiny::renderPlot({
      ...
      tryCatch({
        if (analysis_type == "outcomes") {
          ...
          do.call(openqaly::twsa_outcomes_plot, args)
          ...
          do.call(openqaly::twsa_costs_plot, args)
          ...
          args$interventions <- input$interventions
          args$comparators <- input$comparators
          do.call(openqaly::twsa_nmb_plot, args)
          ...
          args$interventions <- input$interventions
          args$comparators <- input$comparators
          do.call(openqaly::twsa_ce_plot, args)
          ...
          if (!is.null(input$groups)) args$groups <- input$groups
          do.call(openqaly::twsa_vbp_plot, args)
        }
      }, error = function(e) {
    ...
    output$result_table <- shiny::renderUI({
      ...
      tryCatch({
        ft <- if (analysis_type == "outcomes") {
          ...
          do.call(openqaly::twsa_outcomes_table, args)
          ...
          do.call(openqaly::twsa_costs_table, args)
          ...
          args$interventions <- input$interventions
          args$comparators <- input$comparators
          do.call(openqaly::twsa_nmb_table, args)
          ...
          args$interventions <- input$interventions
          args$comparators <- input$comparators
          do.call(openqaly::twsa_ce_table, args)
          ...
          if (!is.null(input$vbp_comparators)) args$comparators <- input$vbp_comparators
          if (!is.null(input$groups)) args$groups <- input$groups
          do.call(openqaly::twsa_vbp_table, args)
        }
